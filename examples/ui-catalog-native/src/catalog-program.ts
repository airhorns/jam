type Binding = Record<string, string | number | boolean>;
type VNode = unknown;

declare const $: { value: unknown };
declare function createJamUI(config: unknown): void;
declare function h(tag: unknown, props?: Record<string, unknown> | null, ...children: unknown[]): VNode;
declare function replace(...terms: (string | number | boolean)[]): void;
declare function when(...patterns: unknown[][]): Binding[];
...
declare const XStack: any;
declare const YStack: any;

export function configureCatalogTheme(): void {
  createJamUI({
    tokens: {
...
function read(key: string, fallback: string | boolean): string | boolean | number {
  const matches = when(["catalog", key, $.value]);
  return matches.length > 0 ? matches[0].value : fallback;
}

function Section(title: string, detail: string, body: VNode): VNode {
  return h(Card, {
    padding: "$space.5",
    gap: 12,
...
function Swatch(color: string, label: string): VNode {
...
export function Catalog(): VNode {
  const accepted = Boolean(read("accepted", true));
  const notifications = Boolean(read("notifications", false));
  const choice = String(read("choice", "native"));
  const tab = String(read("tab", "overview"));
  const progress = notifications ? 82 : 48;

  return h(ScrollView, { id: "catalog-root", flex: 1, backgroundColor: "$background" },
      ...
      Section("Controls", "Buttons, inputs, switches, checkboxes, radios, and sliders.",
        h(YStack, { gap: 16 },
          h(XStack, { gap: 10, flexWrap: "wrap" },
            h(Button, { id: "primary-button", onClick: () => { replace("catalog", "tab", "overview"); } }, h(Text, {}, "Primary")),
            h(Button, { id: "outlined-button", variant: "outlined", onClick: () => { replace("catalog", "tab", "native"); } }, h(Text, {}, "Outlined")),
            h(Button, { variant: "ghost" }, h(Text, {}, "Ghost")),
          ),
          h(YStack, { gap: 8 },
            h(Label, {}, "Name"),
            h(Input, { id: "name-input", placeholder: "Ada Lovelace" }),
            h(TextArea, { id: "notes-input", placeholder: "Native review notes" }),
          ),
          h(XStack, { gap: 12, alignItems: "center" },
            h(Checkbox, { id: "accepted", checked: accepted, onCheckedChange: (next: boolean) => { replace("catalog", "accepted", next); } },
              h(Checkbox.Indicator, {}, h(Text, {}, "ok")),
            ),
            h(Text, {}, accepted ? "Accepted" : "Not accepted"),
            h(Switch, { id: "notifications", checked: notifications, onCheckedChange: (next: boolean) => { replace("catalog", "notifications", next); } }),
          ...
          h(RadioGroup, { value: choice, orientation: "horizontal" },
            h(XStack, { gap: 10, alignItems: "center" },
              h(RadioGroup.Item, { id: "radio-web", value: "web", checked: choice === "web", onSelect: () => { replace("catalog", "choice", "web"); } },
                choice === "web" ? h(RadioGroup.Indicator, {}) : null,
              ),
              h(Text, {}, "Web"),
              h(RadioGroup.Item, { id: "radio-native", value: "native", checked: choice === "native", onSelect: () => { replace("catalog", "choice", "native"); } },
                choice === "native" ? h(RadioGroup.Indicator, {}) : null,
              ),
              h(Text, {}, "Native"),
            ),
          ),
          h(Slider, { id: "progress-slider", value: [progress], min: 0, max: 100 }),
        ),
      ),

      Section("Composition", "Tabs, progress, scroll containers, separators, and card sections.",
        h(YStack, { gap: 14 },
          h(Progress, { id: "progress", value: progress, max: 100 },
            h(Progress.Indicator, { width: `${progress}%` }),
          ),
          h(Separator, {}),
          h(Tabs, { value: tab },
            h(Tabs.List, {},
              h(Tabs.Tab, { id: "tab-overview", onClick: () => { replace("catalog", "tab", "overview"); } }, h(Text, {}, "Overview")),
              h(Tabs.Tab, { id: "tab-native", onClick: () => { replace("catalog", "tab", "native"); } }, h(Text, {}, "Native")),
            ),
            h(Tabs.Content, {},
...
export const catalogSetupSource = `(${configureCatalogTheme.toString()})();`;
export const catalogMountSource = [
  read.toString(),
  Section.toString(),
  Swatch.toString(),
  Catalog.toString(),
