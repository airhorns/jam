...
BEGIN
    ...
    IF is_syncing THEN
        ...
        IF (OLD.synced = TRUE) OR (OLD.sent_to_server = TRUE AND NEW.modified >= OLD.modified) THEN
            ...
            NEW.sent_to_server := FALSE;
        ELSE
            -- Keep locally modified columns as they are; take the server's value for
            -- every other column and remember what it replaced. NEW is edited in place:
            -- a nested UPDATE here would re-fire this trigger on the same row forever.
            FOR column_name IN SELECT columns.column_name
                ...
                IF column_name = ANY(OLD.modified_columns) THEN
                    NEW := jsonb_populate_record(NEW, jsonb_build_object(column_name, old_value));
                ELSIF new_value IS DISTINCT FROM old_value THEN
