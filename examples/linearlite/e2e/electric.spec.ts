...
async function open(page: Page, path = `/${WEB}`) {
  ...
  await expect(page.getByTestId("sync-badge")).toHaveText("Synced with Electric", { timeout: 20000 });
}
...
test.describe("LinearLite on Electric", () => {
  ...
  test("pushes local edits to Postgres and reloads them from the local mirror", async ({ page }) => {
    ...
    await page.getByTestId("issue-title").fill("Renamed through Electric");
    ...
    await expect(page.getByTestId("sync-badge")).toHaveText("Synced with Electric");
    await expectMirrored(page, scopeOf(WEB));

    await page.reload();
    await expect(page.getByTestId("issue-title")).toHaveValue("Renamed through Electric", { timeout: 15000 });
    await expect(page.getByTestId("sync-badge")).toHaveText("Synced with Electric", { timeout: 20000 });
    await expectMirrored(page, scopeOf(WEB));
  });
  ...
  test("shows rows written straight into Postgres", async ({ page }) => {
    ...
    await expect(page.getByTestId("issue-row-title").first()).toHaveText("Arrived from Postgres");
    ...
    await expect(page.locator(`[data-testid='issue-row'][data-issue-id='${id}'] [data-testid='issue-row-title']`)).toHaveText("Retitled in Postgres");
    await expectMirrored(page, scopeOf(WEB));
  ...
  test("two browsers converge on the same project", async ({ browser }) => {
    ...
    await expect(b.locator(`[data-testid='issue-row'][data-issue-id='${id}'] [data-testid='issue-row-title']`)).toHaveText("Edited in browser A", { timeout: 15000 });

    await b.locator(`[data-testid='issue-row'][data-issue-id='${id}'] [data-testid='issue-row-title']`).click();
